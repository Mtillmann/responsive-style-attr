...
The "headless" variant lets you generate stylesheets off of document fragments. It does not rely on the DOM so you can run it in a node environment. Since `classList` is not available, data-attributes and data-attribute selectors are generated. The headless variant also supports an extra option
...
```
You can call `parse` on the same instance repeatedly or pass an entire document. `parse`'s output will be the fragment but with selector attributes added. 

Adding styles directly is also supported, simply pass an object or json string to the `push` method and receive a list of attributes (generated by `options.selectorPropertyAttacher`):
